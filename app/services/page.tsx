        ...
        <section className="section" style={{ backgroundColor: '#0D0D10', borderTop: '1px solid #1C1C22' }}>
          <div className="max-w-7xl mx-auto px-6">
            <FadeUp className="mb-16 text-center">
              <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: '#DC2626' }}>How We Work</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-px" style={{ backgroundColor: '#1C1C22' }}>
              {process.map((p, i) => (
                <FadeUp key={p.step} delay={i * 0.1}>
                  <div className="p-8" style={{ backgroundColor: '#0D0D10' }}>
                    <span className="block text-4xl font-black mb-4" style={{ color: 'rgba(220,38,38,0.3)', fontFamily: 'monospace' }}>{p.step}</span>
                    <h3 className="text-lg font-bold text-white mb-3">{p.title}</h3>
                    <p className="text-sm leading-relaxed" style={{ color: '#71717A' }}>{p.description}</p>
                  </div>
                </FadeUp>
              ))}
            </div>
